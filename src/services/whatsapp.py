  ...
  async def send_text_humanized(
    ...
    if len(paragraphs) == 1:
        ...
        for line in lines:
            ...
            if (
                line[0].isdigit() and ". " in line[:4] or  # Numbered list item
                line.startswith(("- ", "• ")) or  # Bullet point
                len(current_chunk) >= 3  # Max 3 lines per chunk
            ):
                paragraphs.append("\n".join(current_chunk))
                current_chunk = []
  ...
  async def send_template(
    ...
    async with httpx.AsyncClient() as client:
      ...
      if response.status_code != 200:
        logger.error(f"Error sending template message to {to}: {response.text}")
        raise Exception(f"Error sending template message to {to}: {response.text}")
